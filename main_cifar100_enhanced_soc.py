...
conv_mapping = {'soc':skew_conv, 'standard':nn.Conv2d}
...
class AlexNet(nn.Module):
    def __init__(self,taskcla,conv_module):
        ...
        self.conv1_standard = nn.Conv2d(64, 64, 4, bias=False)
        self.bn11 = nn.BatchNorm2d(64, track_running_stats=False)
        ...
        self.conv2_standard = nn.Conv2d(128, 128, 3, bias=False)
        self.bn21 = nn.BatchNorm2d(128, track_running_stats=False)
        ...
        self.conv3_standard = nn.Conv2d(256, 256, 2, bias=False)
        self.bn31 = nn.BatchNorm2d(256, track_running_stats=False)
    ...
    def forward(self, x):
        ...
        self.act['conv1']=x
        x = self.bn11(self.conv1_standard(self.relu(x)))
        x = self.maxpool(self.drop1(self.relu(x)))

        x = self.bn2(self.conv2(x))
        self.act['conv2']=x
        x = self.bn21(self.conv2_standard(self.relu(x)))
        x = self.maxpool(self.drop1(self.relu(x)))

        x = self.bn3(self.conv3(x))
        self.act['conv3']=x
        x = self.bn31(self.conv3_standard(self.relu(x)))
...
def get_representation_matrix (net, device, x, y=None): 
    ...
    for i in range(len(net.map)):
        ...
        if i<3:
            ...
            #print('bsz={},s={}'.format(bsz,s))
            for kk in range(bsz):
                for ii in range(s):
                    for jj in range(s):
                        #print(np.shape(act[kk,:,ii:ksz+ii,jj:ksz+jj]))
...
def main(args):
    ...
    for k,ncla in taskcla:
        ...
        if task_id==0:
            conv_module = conv_mapping[args.conv_type]
            model = AlexNet(taskcla, conv_module).to(device)
...
if __name__ == "__main__":
    ...
    # Model parameters
    parser.add_argument('--conv_type', type=str, default='standard', metavar='CONV',
                        help='conv type (default: standard)')
